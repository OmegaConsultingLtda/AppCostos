    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="categoryFilter" className="text-sm text-text-secondary">Categoría:</label>
          <select
            id="categoryFilter"
            className="bg-surface border border-gray-200 dark:border-gray-700 text-text-primary rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-accent"
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-background text-text-secondary text-sm uppercase tracking-wider">
                ...
                <th className="p-3 font-semibold cursor-pointer hover:text-text-primary" onClick={() => handleSort('date')}>
                  <div className="flex items-center gap-1">Fecha {getSortIcon('date')}</div>
                </th>
                <th className="p-3 font-semibold hidden sm:table-cell cursor-pointer hover:text-text-primary" onClick={() => handleSort('category')}>
                   <div className="flex items-center gap-1">Categoría {getSortIcon('category')}</div>
                </th>
                <th className="p-3 font-semibold cursor-pointer hover:text-text-primary text-right" onClick={() => handleSort('amount')}>
              ...
              {transactions.length > 0 ? (
                transactions.map(tx => (
                  <tr key={tx.id} className="hover:bg-background/50 transition-colors">
                    <td className="p-3 font-medium text-text-primary">{tx.description}</td>
                    <td className="p-3 text-sm text-text-secondary">{formatYmdToDmy(tx.date)}</td>
                    <td className="p-3 hidden sm:table-cell text-text-secondary">
                      {tx.category}
                      {tx.subcategory && <span className="block text-xs text-text-secondary">{tx.subcategory}</span>}
                    </td>
                    <td className={clsx("p-3 text-right font-bold", tx.type === 'income' ? 'text-brand-accent' : 'text-brand-secondary')}>
                    ...
                    <td className="p-3 text-center">
                      <button 
                        ...
                        className="text-brand-primary hover:text-brand-primary/80 mr-2"
                      >
                        Editar
                      </button>
                      <button 
                ...
                <tr>
                  <td colSpan={6} className="p-8 text-center text-text-secondary">No hay movimientos en este período.</td>
                </tr>
              )}
            </tbody>
            <tfoot>
              <tr className="bg-background text-text-primary text-sm">
                <td className="p-3 font-semibold" colSpan={4}>Total Neto</td>
                <td className={clsx("p-3 text-right font-bold", netTotal >= 0 ? 'text-brand-accent' : 'text-brand-secondary')}>
